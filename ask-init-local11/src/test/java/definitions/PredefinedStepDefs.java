...
public class PredefinedStepDefs {
    ...
    public void iWaitForElementWithXpathToNotBePresent(String xpath) {
        new WebDriverWait(getDriver(), 15, 200).until(ExpectedConditions.not(ExpectedConditions.presenceOfElementLocated(By.xpath(xpath))));
    }
    ...
    public void iScrollToTheElementWithXpathWithOffset(String xpath, int offset) throws Exception {
        JavascriptExecutor executor = (JavascriptExecutor) getDriver();
        executor.executeScript("arguments[0].scrollIntoView(false);", getDriver().findElement(By.xpath(xpath)));
        executor.executeScript("window.scrollBy(0, " + offset + ");", getDriver().findElement(By.xpath(xpath)));
        Thread.sleep(1000);
    }
